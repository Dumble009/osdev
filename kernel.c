...
/// @brief ディスクデバイスに新しいリクエストの存在を通知。
/// @param vq リクエストを保持するvirtqueue
/// @param desc_index 新しいリクエストの先頭ディスクリプタのインデックス
...
/// @brief 現在キューの中にデバイスが処理中のリクエストがあるかどうか
/// @param vq チェックしたいキュー
/// @return 処理中のリクエストがあればtrue、そうでなければfalse
bool virtq_is_busy(struct virtio_virtq *vq)
{
    return vq->last_used_index != *vq->used_index;
}

/// @brief virtio-blkデバイスを読み書きする
/// @param buf データが格納されている/データを格納するバッファ
/// @param sector 何セクタ目を読み書きしたいのか
/// @param is_write 書き込み処理ならtrue、読み込み処理ならfalse
void read_write_disk(void *buf, unsigned sector, int is_write)
{
    if (sector >= blk_capacity / SECTOR_SIZE)
    {
        printf("virtio: tried toread/write sector=%d, but capacity is %d\n", sector, blk_capacity / SECTOR_SIZE);
        return;
    }

    printf("read_write_disk: start\n");
    ...
    printf("read_write_disk: finish build requests\n");
    ...
    printf("read_write_disk: finish build discriptors\n");

    // デバイスに新しいリクエストの存在を通知
    virtq_kick(vq, 0);

    printf("read_write_disk: finish kick\n");
    ...
    printf("read_write_disk: finish waiting device\n");
    ...
    // 読み込み処理の場合はバッファにデータをコピーする
    if (!is_write)
    {
        memcpy(buf, blk_req->data, SECTOR_SIZE);
    }

    printf("read_write_disk: finish the function\n");
}
...
/// @brief 8進数から10進数への変換
/// @param oct 8進数値を持つ文字列
/// @param len octの長さ。ヌル文字の文は考慮しない
/// @return octを10進数に変換した数値
...
void fs_init(void)
{
    ...
    for (int i = 0; i < FILES_MAX; i++)
    {
        ...
        off += (uint32_t)align_up(sizeof(struct tar_header) + filesz, SECTOR_SIZE);
    }
}

/// @brief ディスクへのデータの書き戻し
void fs_flush(void)
{
    // files変数の各ファイルの内容をdisk変数に書き込み、
    // それをディスクイメージに書き込む
